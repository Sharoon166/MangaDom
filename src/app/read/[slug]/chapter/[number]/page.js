                    ...
                    <div key={index} className="mb-4 w-full max-w-3xl relative">
                        {!loadedImages.includes(index) && (
                            <div className="absolute inset-0 flex items-center justify-center bg-gray-100 dark:bg-gray-800">
                                <div className="animate-pulse">Loading image {index + 1}...</div>
                            </div>
                        )}
                        <Image
                            src={image}
...
const ChapterNavigation = ({ prevChapter, nextChapter, mangaSlug, currentChapter, mangaTitle, allChapters }) => {
    const currentIndex = allChapters.findIndex(ch => ch.attributes.chapter === currentChapter);
        ...
        <div className="my-8 bg-background p-4 rounded-lg border shadow-lg z-10">
            ...
            <Pagination>
                <PaginationContent>
                    {prevChapter ? (
                        <PaginationItem>
                            <PaginationPrevious href={`/read/${mangaSlug}/chapter/${prevChapter}`} />
                        </PaginationItem>
                    ) : (
                        <PaginationItem>
                    ...
                    {allChapters.slice(Math.max(0, currentIndex - 1), currentIndex).map((chapter) => (
                        <PaginationItem key={chapter.id}>
                            <PaginationLink href={`/read/${mangaSlug}/chapter/${chapter.attributes.chapter}`}>
                    ...
                    {/* Next chapters */}
                    {allChapters.slice(currentIndex + 1, currentIndex + 2).map((chapter) => (
                        <PaginationItem key={chapter.id}>
                            <PaginationLink href={`/read/${mangaSlug}/chapter/${chapter.attributes.chapter}`}>
                                {chapter.attributes.chapter}
                            </PaginationLink>
                    ...
                    {nextChapter ? (
                        <PaginationItem>
                            <PaginationNext href={`/read/${mangaSlug}/chapter/${nextChapter}`} />
                        </PaginationItem>
                    ) : (
                        <PaginationItem>
...
export default function ChapterPage() {
    ...
    async function findMangaBySlug(slug) {
        try {
            ...
            const response = await fetch(`https://api.mangadex.org/manga?originalLanguage[]=ja&availableTranslatedLanguage[]=en&contentRating[]=safe&contentRating[]=suggestive&contentRating[]=erotica&includes[]=cover_art&includes[]=author&order[relevance]=desc&limit=5&title=${encodeURIComponent(slug)}`, {
                headers: {
            ...
            if (data.data && data.data.length > 0) {
                const exactMatch = data.data.find(manga =>
                    manga.attributes.title.en?.toLowerCase() === slug.replace(/-/g, ' ').toLowerCase() ||
                    Object.values(manga.attributes.title).some(title =>
            ...
            const fallbackResponse = await fetch(`https://api.mangadex.org/manga?originalLanguage[]=ja&availableTranslatedLanguage[]=en&includes[]=cover_art&includes[]=author&order[relevance]=desc&limit=5&title=${encodeURIComponent(slug.replace(/-/g, ' '))}`, {
                headers: {
                    'Accept': 'application/json',
                },
                cache: 'no-store'
        ...
        async function fetchChapterFromMangaDex() {
            try {
                ...
                const validChapters = chaptersData.data.filter(
                    chapter => chapter.attributes.pages > 0 && chapter.attributes.chapter
                )
                ...
                const prevChapter = chapterIndex > 0
                    ? validChapters[chapterIndex - 1].attributes.chapter
                    : null

                const nextChapter = chapterIndex < validChapters.length - 1
                    ? validChapters[chapterIndex + 1].attributes.chapter
                    : null
    ...
    if (error || !chapterData) {
        return (
            <div className="container mx-auto px-4 py-8 flex flex-col justify-center items-center min-h-screen">
                <div className="text-xl text-destructive mb-4">{error || 'Chapter not found'}</div>
                ...
                <div className="mt-8 p-4 bg-muted rounded-lg max-w-lg text-center">
                    <p className="text-sm text-muted-foreground">
                        We're having trouble connecting to MangaDex. This could be due to network issues,
                        API rate limiting, or the manga may not be available. Please try again later.
                    </p>
                </div>
            </div>
        )
    ...
    return (
        <div className={`container mx-auto px-4 py-8 ${readingDirection === 'rtl' ? 'rtl' : 'ltr'}`} ref={contentRef}>
            <ReaderSettings
                dataSaver={dataSaver}
            ...
            <div className="flex justify-center pointer-events-none">
                <div className="bg-primary text-primary-foreground px-4 py-2 rounded-full text-sm">
                    {chapterImages.length > 0 ? `${chapterData.number} • ${chapterImages.length} Chapters` : 'Loading pages...'}
                </div>
            </div>
        </div>
    )
