...
export async function generateMetadata({ params }) {
    ...
    return {
        ...
        description: manga.attributes.synopsis?.substring(0, 160) || "Explore this manga on Mangadom",
    };
}
...
export default async function MangaPage({ params }) {
    ...
    const japaneseTitle = attributes.titles.ja_jp ||
        attributes.titles.ja;

    const description = attributes.synopsis || "No description available";

    const coverUrl = attributes.coverImage?.original ||
        attributes.posterImage?.original ||
        null;

    const posterUrl = attributes.posterImage?.original ||
                ...
                <div className="flex flex-col md:flex-row gap-8">
                    ...
                    <div className="flex-1">
                        ...
                        <div className="mb-6 flex flex-wrap gap-4 text-sm">
                            {attributes.subtype && (
                                <div>
                                    ...
                                    <span className="font-medium capitalize">{attributes.subtype}</span>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-3 mb-6">
                                    ...
                                    <div className="flex">
                                        {[...Array(5)].map((_, i) => (
                                            <svg
                                                ...
                                                className={`h-4 w-4 ${i < Math.round(parseFloat(rating))
                                                    ? "text-yellow-500"
                            ...
                            {attributes.status && (
                                <div className={`px-3 py-1 justify-center items-center rounded-full flex  text-white text-sm font-medium ${statusColors[attributes.status.toLowerCase()] || "bg-gray-500"}`}>
                            ...
                            {attributes.ageRating && (
                                <div className="px-3 py-1 flex items-center justify-center rounded-full bg-purple-500 text-white text-sm font-medium">
                                    {attributes.ageRating}{attributes.ageRatingGuide ? ` (${attributes.ageRatingGuide})` : ''}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                            ...
                            <div className="bg-card p-4 rounded-xl shadow-sm border border-border text-center">
                                <div className="text-muted-foreground text-sm mb-1">Chapters</div>
                                <div className="text-2xl font-bold">
                            ...
                            <div className="bg-card p-4 rounded-xl shadow-sm border border-border text-center">
                                <div className="text-muted-foreground text-sm mb-1">Volumes</div>
                                <div className="text-lg font-medium">
                        ...
                        <div className="bg-card rounded-xl shadow-sm p-6 border border-border">
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <div className="font-medium capitalize">{attributes.status?.replace(/_/g, ' ') || "Unknown"}</div>
                                </div>
                                <div>
                                    <div className="text-muted-foreground text-sm">Age Rating</div>
                                    <div className="font-medium">
                                        {attributes.ageRating || "Unknown"}
                                        {attributes.ageRatingGuide && (
                                            <span className="text-xs text-muted-foreground ml-1">({attributes.ageRatingGuide})</span>
                                        )}
                                    </div>
                                </div>
                                <div>
                                    <div className="text-muted-foreground text-sm">Type</div>
                                    <div className="font-medium capitalize">{attributes.subtype || "Unknown"}</div>
                                </div>
                                <div>
                                    <div className="text-muted-foreground text-sm">Chapter Count</div>
                                    <div className="font-medium">
                                        {attributes.chapterCount || "Ongoing"}
                                    </div>
                                </div>
                                <div>
                                    <div className="text-muted-foreground text-sm">Volume Count</div>
                                    <div className="font-medium">
                                        {attributes.volumeCount || "Ongoing"}
                                    </div>
                                </div>
